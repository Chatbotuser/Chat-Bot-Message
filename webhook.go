...
func webhookGetHandler(w http.ResponseWriter, r *http.Request) {
	...
	// connect webhook false edit token =" verify_token"
	if tokenTrue == token {
		...
		for _, entries := range body.Entry {
			for _, messaging := range entries.Messaging {
				if messaging.Message != nil {
					...
					echo := messaging.Message.Is_echo

					// kiem tra nếu người dùng có đính kèm phải
					if attachments != nil {
						// Send message không thể xử lý tệp đính kèm
						...
						vr := hearStruct{text: text, echo: echo}
						vr.listen(messaging.Sender.ID, messaging.Recipient.ID)
					}

				} else if messaging.Postback != nil {

					if messaging.Postback.Payload == "aws" {
						// Sự kiện người dùng lần đầu chat button get started
						SendMessage(messaging.Sender.ID, "Hello! It seems this is the first time we're talking! Call me JP!")
					}
